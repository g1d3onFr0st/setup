...
# Ordered field names (matching METADATA_FORMAT)
METADATA_FIELDS = [
    "player",
    "status",
    "title",
...
STATE_REFRESH_INTERVAL = 0.75  # Periodically resync metadata/player discovery
...
def toolbar_row():
    ...
    tools = "  ".join([seek, play_pause, prev, next_, close])

    gaps_w = 4 * 2
    tool_w = 7 + 7 + 4 + 4 + 11 + gaps_w
...
def update_state_from_metadata(data: dict, force: bool = False):
    """Update state from parsed metadata dict.

    force=True bypasses command debounce for an explicit authoritative refresh.
    """

    if not force and time.time() - s.last_command_time < COMMAND_DEBOUNCE:
        return
    changed = False
...
def _query_position_seconds() -> Optional[float]:
    """Return the player's current position in seconds, or None on failure."""
    raw = run_playerctl("position").strip()
    if not raw:
        return None
    try:
...
def _query_length_seconds() -> Optional[float]:
    """Return the current track length in seconds, or None on failure.

    MPRIS exposes mpris:length in microseconds, so query that property directly
    instead of trusting the metadata follower's templated length field.
    """
    raw = run_playerctl("metadata", "mpris:length").strip()
    if not raw:
        return None
    try:
...
def refresh_current_state(force: bool = True) -> bool:
    """Refresh metadata plus authoritative playback timing from playerctl.

    Metadata and timing intentionally have separate owners:
      * metadata snapshot/follower: title, artist, album, shuffle, etc.
      * dedicated playerctl queries: position and mpris:length

    This prevents empty timing fields from the metadata follower from repeatedly
    overwriting valid progress with 0:00 / Live after a seek.
    """
    if not s.current_player:
        return False

    changed = False

    # Refresh descriptive metadata, but NEVER let the templated metadata path
    # own position/length. Those are queried authoritatively below.
    raw = run_playerctl("--format", METADATA_FORMAT, "metadata")
    if raw:
        data = parse_metadata(raw)
        if data:
            data.pop("position", None)
            data.pop("length", None)
            before_dirty = s.state.dirty
            update_state_from_metadata(data, force=force)
            changed = changed or s.state.dirty or before_dirty

    status = run_playerctl("status").strip()
    if status and s.state.status != status:
        s.state.status = status
        changed = True

    position = _query_position_seconds()
    if position is not None and s.state.position != position:
        s.state.position = position
        changed = True

    length = _query_length_seconds()
    if length is not None and s.state.length != length:
        s.state.length = length
        changed = True

    s.state.player = s.current_player
    if changed or raw or status:
        s.state.dirty = True
        return True
    return False

def ensure_player_and_refresh() -> None:
    """Discover newly appearing/disappearing players and refresh current state."""
    players = get_available_players()

    if not players:
        if s.current_player or s.state.status != "No MPRIS player":
            cleanup_proc(s.meta_proc)
            s.meta_proc = None
            s.current_player = ""
            s.current_player_idx = -1
    ...
    s.available_players = players

    if s.current_player not in players:
        cleanup_proc(s.meta_proc)
        s.current_player = get_best_player(players) or players[0]
        s.current_player_idx = players.index(s.current_player)
        reset_state()
        s.state.player = s.current_player
        s._meta_buf = ""
        s._initial_state_shown = False
        s.meta_proc = start_metadata_follower()
    elif s.meta_proc is None or s.meta_proc.poll() is not None:
        cleanup_proc(s.meta_proc)
        s._meta_buf = ""
        s._initial_state_shown = False
        s.meta_proc = start_metadata_follower()

    refresh_current_state(force=True)
...
def handle_key(key: str, seq: str = "") -> None:
    ...
    if key == "\x1b":
        if seq == "[C":
            ...
            return
        # Work around the player's broken post-seek MPRIS state.
        #
        # Manually pressing Space twice is known to make this player publish
        # correct timing/metadata again. Reproduce that exact operation here:
        # toggle once, allow MPRIS to settle, refresh; toggle back, allow it to
        # settle again, then refresh and redraw. Two toggles preserve the
        # original playing/paused state.
...
def read_metadata_from_follower(raw: str) -> None:
    ...
    for block in complete_blocks:
        ...
        if parsed:
            # The follower owns descriptive metadata only. Templated position/
            # length can be empty during seeks and must never zero the progress.
